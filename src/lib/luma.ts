...
// sections opt in with a `data-bg-luma="dark"|"light"` attribute on their
// common ancestor, checked before the colour walk.
export function solidBgFrom(node: Element | null): TBackdrop | null {
  while (node) {
    const override = node.getAttribute(BG_LUMA_ATTRIBUTE);
    ...
    // mark such layers with data-bg-luma to keep readers adaptive.
